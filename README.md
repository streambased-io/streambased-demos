...
1. [Slow Data](/cases/1_slow_data) - What is the e2e latency of your data? By this we mean after a datapoint is created, how long does it
   take for it to be available for analytics? Hours? Days? Weeks? If it's larger than zero you need
   Streambased.
2. [Real-time FOMO](/cases/2_fomo) - Are your competitors benefiting from datasets you can't see? Real-time data powers the world but
...
3. [Kafka is a Black Box](/cases/3_black_box) - Kafka is opaque! Messages go in and messages come out but what happens in the middle is largely
   ...
   data.
4. [Two Generals](/cases/4_two_generals) - The two generals problem https://en.wikipedia.org/wiki/Two_Generals%27_Problem is a popular CS
...
5. [Making Kafka do Batch](/cases/5_batch) - I have an external customer who's specced a daily batch fetch job for my data. The data is in Kafka,
   how do I give to them?
6. [Introducing Iceberg](/cases/6_all_in_one) - This all-in-one demo shows the entire Streambased product suite including Kafka as a 
   filesystem, Kafka as Iceberg and Kafka directly to BI tools.
7. [Streambased MCP](/cases/7_mcp) - Explore "Vibe coding for data" with Streambased MCP server and client.
8. [Fake Invoice Scam](/cases/8_Fake_Invoice_Scam) - Example data discrepancy investigation with Streambased MCP
9. [Iceberg HotSet + ColdSet union](/cases/9_iceberg_union) - Example of real-time Iceberg with physical ColdSet combined with Logical Projection HotSet using client-side Union.
## Executing a case
